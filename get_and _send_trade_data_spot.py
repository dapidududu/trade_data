...
def send_to_kafka(csv_path, coin, producer, logger):
    ...
    for chunk_num, chunk in enumerate(df_chunks):
        ...
        for message in batch:
            try:
                producer.send(f'{coin}_SPOT', value=message)
...
def download_days_trade_data(coin):
    base_dir = "."  # 数据存放目录
    ...
    # 获取 ZIP 文件列表
    zip_path_list = [(os.path.join(coin_dir, f), coin) for f in os.listdir(coin_dir) if f.endswith(".zip")]
    return zip_path_list


def download_yesterday_trade_data(coin, yesterday_str):
    base_dir = "."  # 数据存放目录
    os.makedirs(coin, exist_ok=True)
    coin_dir = os.path.join(base_dir, coin)
...
def main():
    base_dir = "."  # 数据存放目录
    coin_list = ["BTCFDUSD"]
    yesterday_str = (pd.Timestamp.today() - pd.Timedelta(days=1)).strftime('%Y-%m-%d')
    zip_files = []
    for coin in coin_list:
        zip_path_list = download_days_trade_data(coin)
        # zip_path_list = download_yesterday_trade_data(coin, yesterday_str)

...
def safe_request(url, logger, params=None, max_retry=MAX_RETRY):
    """带有限流和退避机制的请求"""
    retry = 0
    while retry < max_retry:
        try:
            ...
            if resp.status_code in (418, 429):
                ...
                logger.info(f"[WARN] Rate limit hit. Sleeping {wait_time:.2f}s...")
            ...
            logger.info(f"[ERROR] Request failed: {e}, retrying in {wait_time:.2f}s")
            time.sleep(wait_time)
            retry += 1
...
def fetch_funding_rate(symbol, logger, start_time, end_time, limit=1000):
    """获取某个币种的资金费率数据"""
    ...
    data = safe_request(url, logger, params=params)
...
if __name__ == "__main__":
    ...
    fetch_incremental_data(symbols)
